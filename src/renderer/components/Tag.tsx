...
function TagPage() {
  ...
  async function loading(
    ...
    // ジャンル名をセット
    setTagName(
      await window.electron.doGetTagName(TAG_ID)
    );

    let iToalCount:number = await window.electron.doGetItemCountWhereTagID(TAG_ID);
    ...
    let retItems = await window.electron.doGetItemsWhereTag(TAG_ID, PAGE_ITEM_CNT, iOffset);
    setOffset(0);
  ...
  //
  async function handlePageChange(TAG_ID:string|undefined, pageNo:number)
  {
    if( TAG_ID == undefined ) {
      return;
    }
    ...
    loading( parseInt(""+TAG_ID), pageNo);
  }
  ...
  return (
    ...
    <div>
      <Breadcrumb>
        ...
        <Breadcrumb.Item active>{t("commonTitle.tag")}:{tagName}</Breadcrumb.Item>
    </Breadcrumb>
    </div>
      <div>
